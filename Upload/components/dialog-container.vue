  ...
  methods: {
    ...
    async onHandleAdd() {
      ...
      if (this.limit === 1) {
        ...
        this.$emit(
          "getPicD",
          this.list.filter(item => item.checked), checkedList
        );
      }
    ...
    onHandleClickFileLi(index,item) {
      console.log("onHandleClickFileLi ----img---list")
      console.log(item);
      console.log("onHandleClickFileLi ----img---list")
      let checkedList = this.list.filter(item => item.checked);
      if (
...
<style>
.dialog-container .file-list-tab-pane .file-list .file-list-left .infinite-list li img{
