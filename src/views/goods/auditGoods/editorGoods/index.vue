    ...
    >
          ...
          <div class="basic-items">
            ...
            <el-form-item :label="$t('releasephysical.fbt')" prop="subtitle">
              <el-input
                ...
                :placeholder="$t('releasephysical.qsrspms')"
              ></el-input>
            </el-form-item>
      ...
      <div class="goods-attribute">
        ...
        <div class="attribute-block">
          ...
          <el-form-item
            :label="$t('releasephysical.cpgg')"
            class="add-attribute"
          >
            <el-button type="primary" @click="addAttr">{{
              $t('releasephysical.szsx')
            }}</el-button>
          </el-form-item>
      ...
      <div class="goods-set">
        ...
        <div class="set-block">
          ...
          >
            ...
            ></el-input
            >时，商品库存报警
            <span>(可在库存列表中查看明细，预警数据将以红色加粗显示)</span>
          </el-form-item>
          ...
          <el-form-item :label="$t('releasephysical.xsbq')" prop="checkedLabel">
            <el-checkbox-group v-model="ruleForm.checkedLabel" :max="3">
              <el-checkbox
  ...
  data () {
    return {
      ...
      rules: {
        ...
        //   {
        //     required: true,
        //     message: this.$t('releasephysical.qsrspms'),
        //     trigger: 'blur'
        //   }
        // ],
        keywords: [
    ...
    if (
      ...
      getGoodsInfoById({
        ...
        .then(res => {
          ...
          this.IntroList = JSON.parse(res.data.data.content)
          // if(goodsAttribute.content&&goodsAttribute.richList.length<=0){
          //   try {
          ...
          // }else{
          //   // 移动端商品
          //    // me.IntroList=JSON.parse(data.content)
          //    let list = []
          //       // console.log('335335335',);
          //       if(JSON.parse(goodsAttribute.content) instanceof Array){
          //           JSON.parse(goodsAttribute.content).forEach((item,index)=>{
          //             list.push({name:item.name,content:""})
          ...
          //                   }else{
          //                       list[index].content = list[index]?.content.length<=0?`<${item1.tagType} style='text-align: left;color:#999999;font-size:24rpx;margin-bottom:16px'>${item1.value}</${item1.tagType}>`:list[index].content+`<${item1.tagType}>${item1.value}</${item1.tagType}>`
          //                   }
          //               })
          //           })
          //       }
          ...
          this.ruleForm.sequence = res.data.data.list.mch_sort
          choiceClass({
  ...
  methods: {
    ...
    addAttr () {
      ...
      if (attrFlag) {
        ...
        //   this.$message({
        //     message: this.$t('releasephysical.text9'),
        //     type: 'error',
        //     offset: 102
        //   })
        //   attrFlag = true
						...
						<div class="maskNew" id="maskClass" style="z-index: 9999;">
							<div style="width:724px;height:540px;background:rgba(255,255,255,1);border-radius:4px;position: absolute;left: 50%;top: 50%;transform: translate(-50%,-50%);">
								...
								<div style="height: 70px;box-sizing: border-box;border-top: 1px solid #E9ECEF;display: flex;align-items: center;justify-content: flex-end;">
									<a class="maskClose" id="attrMaskQx" href="javascript:;" style="width:70px; height:40px; border:1px solid #D5DBE8; border-radius:4px;display: flex;align-items: center;justify-content: center;color: #6A7076;margin-right: 10px;box-sizing: border-box;background-color: #FFFFFF;">${this.$t(
                    'zdata.off'
                  )}</a>
									<a id="maskSave" class="attrMaskQr" href="javascript:;" style="width:70px;height:40px;background:#2890FF;border-radius:4px;display: flex;align-items: center;justify-content: center;color: #FEFEFE;margin-right: 20px;">${this.$t(
                    'zdata.ok'
                  )}</a>
								</div>
        ...
        $('#maskSave')
          ...
          .bind('click', function () {
            ...
            if (checkAll.length == 0) {
              that.$message({
                message: this.$t('releasephysical.text10'),
                offset: 102,
                type: 'error',
                customClass: 'zZindex'
              })
            ...
            console.log('345754567865456787654567890')
