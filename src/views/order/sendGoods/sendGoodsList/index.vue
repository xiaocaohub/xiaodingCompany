<template>
  <div class="container">
    ...
    <div class="jump-list">
      <el-button class="bgColor el-icon-printer" type="primary" @click="$router.push('/order/sendGoods/bulkShipment')" v-if="detectionBtn(button_list,'批量发货')">{{$t('sendGoodsList.plfh')}}</el-button>
      <el-button class="fontColor"  @click="$router.push('/order/sendGoods/bulkDeliveryRecord')" icon="el-icon-tickets" v-if="detectionBtn(button_list,'批量发货记录')">{{$t('sendGoodsList.plfhjl')}}</el-button>
    </div>
...
>
              ...
              <div class="goods-item">
                ...
                <span><span class="arrt-title">{{$t('orderLists.js')}}：</span>{{ scope.row.needNum}}</span>
          ...
          <template slot-scope="scope">
            <div class="OP-button">
              <div class="OP-button-top">
                <el-button @click="Details(scope.row)"  v-if="detectionBtn(button_list,'订单详情')">{{$t('orderLists.ddxq')}}</el-button> 
                <el-button @click="dialogShow3(scope.row)" v-if="scope.row.status == '待收货' && scope.row.selfLiftingName == '自提'&&detectionBtn(button_list,'核销')" >{{$t('orderLists.hx')}}</el-button>
                <el-button @click="Edit(scope.row)" v-if="['待付款','待发货'].includes(scope.row.status)&&detectionBtn(button_list,'编辑订单')">{{$t('orderLists.bjdd')}}</el-button>
                <el-button @click="Delivery(scope.row)" v-show="['待发货'].includes(scope.row.status)&&detectionBtn(button_list,'商品发货')">{{$t('orderLists.spfh')}}</el-button>
                <el-button @click="dialogShow2(scope.row)"  v-if="scope.row.courier_num && scope.row.courier_num.length>0&&detectionBtn(button_list,'查看物流')" class="logistics">{{$t('orderLists.ckwl')}}</el-button>
              </div>
            </div>
		      </template>
        </el-table-column>
...
<script>
import sendGoodsList from '@/webManage/js/order/sendGoods/sendGoodsList'
export default sendGoodsList
</script>

<style scoped lang="less">
@import '../../../../webManage/css/order/sendGoods/sendGoodsList.less';
